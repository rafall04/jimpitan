...
export function FinanceDashboardPage() {
  ...
  async function postSelectedCollection() {
    if (!selectedCollectionId) {
      toast.error('Select a validated collection first.');
      return;
    }
    try {
  ...
  return (
    <main id="main-content" className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
      <PageHeader
        title="Finance"
        description="Cash, transactions, approvals, and append-only ledger operations."
        actions={
          <div className="flex flex-wrap gap-2">
            ...
            <Button asChild>
              <Link href="/dashboard/finance/ledger">Ledger</Link>
            </Button>
          </div>
        }
      />
      ...
      <section className="grid gap-3 md:grid-cols-5">
        ...
        <AmountMetric label="Cash balance" amount={cashFlowQuery.data?.closingBalance ?? '0'} detail={permissions.has('reports.private.read') ? 'Ledger-derived cash flow' : 'Requires private report access'} />
        <AmountMetric label="Income total" amount={cashFlowQuery.data?.income ?? '0'} detail="Current report period" />
        <AmountMetric label="Expense total" amount={cashFlowQuery.data?.expense ?? '0'} detail="Current report period" />
        <Metric label="Pending validations" value={pendingTransactionsQuery.data?.total ?? '-'} />
        <Metric label="Pending approvals" value={pendingApprovalsQuery.data?.total ?? '-'} />
      </section>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_24rem]">
        <section className="rounded-lg border bg-card p-4">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Read-only append history.</p>
            </div>
            <Button asChild variant="outline" size="sm">
              <Link href="/dashboard/finance/ledger">Open ledger</Link>
            </Button>
          </div>
          {ledgerQuery.isPending ? <Skeleton className="h-48 w-full" /> : null}
          {ledgerQuery.data?.items.length === 0 ? <EmptyState title="No ledger entries" description="Posted transactions will appear here." /> : null}
            ...
            {ledgerQuery.data?.items.map((entry) => (
              <div key={entry.id} className="grid gap-2 py-3 text-sm sm:grid-cols-[6rem_minmax(0,1fr)_9rem_9rem] sm:items-center">
                ...
                <Link href={`/dashboard/finance/transactions/${entry.transactionId}`} className="font-medium hover:underline">
                  #{entry.ledgerSequence} transaction
                </Link>
        ...
        <aside className="space-y-6">
          ...
          <section className="rounded-lg border bg-card p-4">
            <h2 className="text-base font-semibold">Post validated collection</h2>
            <p className="mt-1 text-sm text-muted-foreground">Backend prevents duplicate collection posting by source collection id.</p>
            <div className="mt-4 space-y-3">
              <select value={selectedCollectionId} onChange={(event) => setSelectedCollectionId(event.target.value)} className="flex h-10 w-full rounded-md border bg-background px-3 py-2 text-sm">
                <option value="">Select validated collection</option>
                {collectionsQuery.data?.items.map((collection) => (
                  <option key={collection.id} value={collection.id}>
                    {formatCollectionMode(collection.collectionMode)} - {collection.route.areaName ?? 'All areas'} - {new Date(collection.collectionDate).toLocaleDateString('id-ID')} - {formatCurrencyAmount(collection.totalAmount)}
                  </option>
                ))}
              </select>
              ...
              <select value={selectedCategoryId} onChange={(event) => setSelectedCategoryId(event.target.value)} className="flex h-10 w-full rounded-md border bg-background px-3 py-2 text-sm">
                <option value="">Default Jimpitan category</option>
              ...
              {postedTransactionId ? (
                <Button asChild variant="outline" className="w-full">
                  <Link href={`/dashboard/finance/transactions/${postedTransactionId}`}>Open posted transaction</Link>
                </Button>
              ) : null}
              {!canPost ? <p className="text-sm text-muted-foreground">Posting requires transaction posting permission.</p> : null}
            </div>
          </section>
      ...
      <section className="rounded-lg border bg-card p-4">
        <h2 className="text-base font-semibold">Ledger-derived account balances</h2>
        <div className="mt-3 grid gap-2 md:grid-cols-3">
          {cashBalanceRows.map((account) => (
            <div key={account.cashAccountId} className="rounded-md border p-3 text-sm">
              ...
              <p className="mt-1 text-xs text-muted-foreground">Sequence #{account.ledgerSequence}</p>
            </div>
          ))}
        </div>
      </section>
      <ConfirmDialog
        ...
        title="Post collection to finance"
        description="Posting creates a finance transaction and append-only ledger entry. Duplicate source collection posting is blocked by the backend."
