 ...
 * Deps: CSRF helper.
 * MainFuncs: Verifies cross-site POST rejection and same-origin allowance.
 * SideEffects: None.
 */
...
describe('isSameOriginHeaderSet', () => {
  ...
  it('allows non-browser server requests without origin metadata', () => {
    expect(isSameOriginHeaderSet({ host: 'rt.example.test' })).toBe(true);
  });
});
