...
import { CollectionStatusBadge, formatCollectionModeLabel } from './status-badge';
...
export function SessionCard({ collection, progressPercent }: { collection: CollectionSessionRecord; progressPercent?: number }) {
  const route = collection.route.areaCode ? `${collection.route.areaCode} - ${collection.route.areaName}` : 'Semua wilayah';
    ...
    <article className="rounded-xl border bg-card p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-muted-foreground">{new Date(collection.collectionDate).toLocaleDateString('id-ID')} - {collection.officer.fullName}</p>
          <p className="mt-1 text-xs font-medium text-muted-foreground">{formatCollectionModeLabel(collection.collectionMode)}</p>
        </div>
        <CollectionStatusBadge status={collection.status} />
      </div>
      <div className="mt-4">
        <div className="mb-2 flex items-center justify-between text-xs text-muted-foreground">
          <span>{isBulk ? 'Total sesi' : `${collection.itemCount} rumah terisi`}</span>
          <span>{percent}%</span>
        </div>
